...
(() => {
  ...
  function createRuntime(doc = document) {
    ...
    function setProps(name, overrides) {
      const r = registry.get(name);
      r.propOverrides = overrides && typeof overrides === "object" ? { ...r.propOverrides || {}, ...overrides } : null;
      registry.bump(name);
    }
  ...
  function init() {
    ...
    const api = {
      ...
      __dcBoot: () => {
        ...
        notifyHost();
        startWebSocketClient(runtime, rootName);
      },
    ...
    function startWebSocketClient(runtime, rootName) {
      const wsHost = location.protocol === 'file:' ? 'localhost:3000' : location.host;
      const wsUrl = `ws://${wsHost}`;
      let ws = null;
      function connect() {
        console.log("[dc-realtime] Connecting to WebSocket server:", wsUrl);
        ws = new WebSocket(wsUrl);
        ws.onopen = () => {
          console.log("[dc-realtime] Connected to local companion server!");
        };
        ws.onmessage = (event) => {
          try {
            const msg = JSON.parse(event.data);
            window.dispatchEvent(new CustomEvent("stream-alert", { detail: msg }));
            if (msg.type === "spotify") {
              runtime.setProps(rootName, {
                trackName: msg.data.title,
                artistName: msg.data.artist,
                progress: msg.data.progressStr,
                duration: msg.data.durationStr,
                progressPercent: msg.data.progressPercent,
                albumArt: msg.data.albumArt,
                isPlaying: msg.data.isPlaying
              });
            } else if (msg.type === "twitch") {
              runtime.setProps(rootName, {
                viewerCount: msg.data.viewerCount,
            ...
            console.error("[dc-realtime] Error processing WebSocket message:", err);
          }
        };
        ws.onclose = () => {
          console.warn("[dc-realtime] Connection closed. Reconnecting in 3s...");
          setTimeout(connect, 3000);
        };
        ws.onerror = (err) => {
          console.error("[dc-realtime] WebSocket error:", err);
          ws.close();
        };
      }
      connect();
